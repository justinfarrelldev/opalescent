        ...
        } = declaration
        {
            ...
            if requires_doc && function_doc_comment.is_none() {
                *function_doc_comment = Some(Documentation::from_raw(
                    "Description: Generated integration test function documentation".to_owned(),
                    span,
                ));
            }
        }
...
## Description: Entry function validates wildcard match exhaustiveness ##
entry main = f(args: string[]): int64 =>
...
## Description: Entry function validates nonexhaustive match diagnostics ##
entry main = f(args: string[]): int64 =>
    let msg = Message.Text
...
";
    ...
    fn test_integration_pure_entry_rejected() {
        const SOURCE: &str = "
## Description: Entry function validates pure entry rejection semantics ##

...
use crate::ast::{
    AstNode, BinaryOp, Expr, HotReloadMetadata, LambdaBody, LiteralValue, Stmt, UnaryOp,
};
...
impl Parser {
    ...
    fn parse_primary(&mut self) -> ParseResult<Expr> {
        ...
        let span = token.span;
        let string_lexeme = token.lexeme.as_str();
        ...
        match token_type {
            ...
            TokenType::StringLiteral(value) => {
                self.parse_string_literal(value, string_lexeme, span)
            }
    ...
    fn parse_string_literal(
        ...
        if value.contains('{') {
            self.parse_string_interpolation(raw_lexeme, span)
        } else {
            Ok(Expr::Literal {

...
- 2026-08-23: Split the final Task 23 correction into two semantic commits: `c1ac311` for the generated POSIX process-control runtime/test repair and `abd153a` for refreshed Task 23 evidence. This kept the behavioral repair bisectable while allowing the evidence files to name the actual runtime-fix hash.

- 2026-08-23T06:29:03Z: Removed the premature Task 31 /factory surface from the compiled Task 24 runtime and replaced its coverage with crate-private runtime-only constructors exercised directly from . This keeps Task 24 on the selected data-model scope while still proving hidden stream identity, delivery ordinal, trust conversion, event variants, and diagnostic accounting invariants.
- 2026-08-23T06:29:03Z: Restricted the codegen imported-signature fallback to  only. Routing  through the same path changed the established byte-boolean ABI for Task 16 error-attachment inspectors, so those imports must continue using their dedicated declaration helper.

- 2026-08-23T06:29:20Z: Removed the premature Task 31 TerminalTestScenario and factory surface from the compiled Task 24 runtime and replaced its coverage with crate-private runtime-only constructors exercised directly from src/runtime/terminal_tests.rs. This keeps Task 24 on the selected data-model scope while still proving hidden stream identity, delivery ordinal, trust conversion, event variants, and diagnostic accounting invariants.
- 2026-08-23T06:29:20Z: Restricted the codegen imported-signature fallback to standard.terminal only. Routing standard.system through the same path changed the established byte-boolean ABI for Task 16 error-attachment inspectors, so those imports must continue using their dedicated declaration helper.

...
- 2026-08-22: The corrective Task 21 backend uses a process-global POSIX signal bridge rather than direct handler-side queue mutation: `SIGTSTP` and `SIGCONT` handlers only write one-byte codes into a nonblocking pipe, a helper thread translates those bytes into runtime observations and readiness publications, and the source remains singleton-owned until drop restores the previous handlers.
- 2026-08-22: Safe proof of real suspend/continue behavior needs a helper subprocess, not an in-process test. The child can catch `SIGTSTP`, report `SuspendRequested`, stop only when `acknowledge_suspend` sends `SIGSTOP`, and then resume under a parent-driven `SIGCONT` before explicit `resume_application`.
- 2026-08-22: Task 23 replaced the temporary `allow_terminal_proposal_imports` boolean gate with an authoritative `TerminalPublicApiPrerequisites` inventory. The type checker now opens `standard.system`, `standard.terminal`, and `standard.terminal.chords` by default because Tasks 13-22 are implemented, while focused tests can disable each prerequisite individually and assert the exact missing capability name.
- 2026-08-22: `standard.testing.terminal` needs both boundaries at once: test-only mode must still be enabled, and the Task 23 prerequisite validator must also pass. The narrow Task 16 error-inspector exception remains importable from `standard.system` even when every terminal prerequisite is cleared.
- 2026-08-22: Task 23 separates adoption readiness from runtime readiness. Import/type-check exposure now depends on the complete prerequisite set plus ABI validation, while codegen keeps selected terminal/chord/core/test symbols non-callable with a dedicated runtime-lowering diagnostic instead of the stale `terminal proposal gate not complete` message.
...
- 2026-08-23T06:29:03Z: Task 24 repair kept the selected terminal data model under repository line-count caps by splitting  into focused  and  siblings; the remaining production  now stays under the 1000-line hook while preserving stable inspectors and hidden metadata behavior.
- 2026-08-23T06:29:03Z: Generated-runtime readiness for selected terminal APIs must stay narrower than Rust-side runtime/stdlib coverage. For Task 24, only , , and  were left runtime-ready because they now have real linked C implementations and a compile-and-run integration proof.

- 2026-08-23T06:29:20Z: Task 24 repair kept the selected terminal data model under repository line-count caps by splitting the runtime terminal model into focused formatting and tail-type siblings; the remaining production model file now stays under the 1000-line hook while preserving stable inspectors and hidden metadata behavior.
- 2026-08-23T06:29:20Z: Generated-runtime readiness for selected terminal APIs must stay narrower than Rust-side runtime and stdlib coverage. For Task 24, only terminal_session_options_default, terminal_session_options_validate, and trusted_terminal_output_from_application_text were left runtime-ready because they now have real linked C implementations and a compile-and-run integration proof.

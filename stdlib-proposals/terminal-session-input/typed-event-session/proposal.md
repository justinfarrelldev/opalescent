...
Adoption of this proposal requires, and MUST occur in the same compatible release as, the complete future language/core/standard-library/test-runner prerequisite bundle specified in `../core-prerequisites.md`. Every normative prerequisite in that document MUST be implemented, tested, and enabled in that release. Those prerequisites normatively define legacy stdin/stdout coordination, one affine generic wait set, monotonic timers, the separate process-control source, `using` cleanup, immutable error propagation and attachments, and test-only availability. This proposal references those rules and adds only terminal-specific behavior. V1 MUST NOT be adopted, advertised, shipped, or reviewed as implementation-ready unless the entire bundle lands in the same compatible release. Partial implementation or staged shipping of the selected terminal API without the bundle is non-conforming. Its approval-ready claim is conditional on exact same-release adoption of that bundle. Until then, no signature or behavior in the current standard library is changed by this proposal, and this proposal claims no current compiler, runtime, standard-library, or test-runner support.
...
Error values follow the same rule. Refining `error is TerminalSessionOpenError.InvalidOptions into invalid` proves stable family and variant identity. An `errors A, B` clause is a nominal family set, not a structural union; unknown additive variants remain propagatable members of their known family. A heterogeneous guard error binding has the compiler-known immutable nominal union and refinement/propagation rules in `../core-prerequisites.md`: an unrefined propagation requires every member family in the enclosing `errors` clause, while a refined branch requires only its narrowed family. Nested handler work is checked independently against that enclosing clause, and preserving the original bound error when nested work fails requires explicit `cause` syntax.
...
- a POSIX process-control source separate from terminal input, with a core-owned nonblocking `Notification | Idle` Poll result;
- affine resource declarations naming cleanup operations, cleanup error effects, and any explicit cleanup-authority transfer operations; `using` acquisition/body/cleanup effect union, scope/borrow escape restrictions, cleanup on every lexical exit, explicit-close cleanup consumption, reverse acquisition order, and the complete body/cleanup error matrix;
- heterogeneous nominal guard-error unions and refinement, independently checked nested handler failures, `propagate error_value` forwarding one already-evaluated immutable error value, and explicit `propagate error_value cause prior_error` / `propagate call() cause prior_error` attachment semantics;
...
`TerminalSession` is one affine, noncopyable, non-publicly-constructible resource type. V1 has no state-indexed session types and makes no typestate claim. Its affine resource declaration registers `terminal_session_close_sync` as cleanup, `TerminalSessionRestoreError` as the cleanup error effect, and cleanup-only `CloseRestorePending` as the sole process-coordinator cleanup-authority transfer result. The compiler enforces the prerequisite's owner/borrow escape restrictions and exactly one cleanup obligation; the runtime validates the binding's current `TerminalSessionState` on every operation before host inspection, input consumption, output mutation, decoder mutation, ledger mutation, or state transition.
...
A wake is a hint. The caller compares the wake source identity/generation as required by the generic wait-set contract and reattempts the source-specific operation. After a terminal wake, the caller invokes `terminal_session_read_event_sync(..., TerminalWait.Poll, ...)`. One successful call returns exactly one normalized event. For each terminal wake, the caller MUST choose a strict positive finite event budget before the first Poll and may process no more than that many ordinary events. It returns to the shared wait set immediately on `TimedOut`, `Cancelled`, `EndOfInput`, or a state error, and MUST also return when the budget is exhausted even if terminal readiness remains asserted. `TimedOut` before budget exhaustion is the required stale/drained-readiness result. A still-ready terminal source remains level-ready and can be reported `Ready` again; therefore repeated bounded drains continue progress without requiring a new edge, while bounded-fair shared-wait selection gives cancellation, process control, and timers an opportunity under sustained input. The API returns one event per successful call and never returns a hidden batch.
...
## Project-first TDD fixture obligations

Before implementing terminal-session runtime, standard-library, compiler, or test-runner support, implementors MUST first write the terminal project fixtures below as complete Opalescent projects under `test-projects/`. Each project MUST use the real selected public signatures and proposed syntax from this proposal, `../core-prerequisites.md`, and, for chord-specific fixtures only, `../CHORDS.md`; placeholder pseudocode, host-language shims, editor-framework assumptions, callback APIs, unproposed language features, or private runtime escape hatches are non-conforming. The test suite MUST register every project as a normal build-and-run fixture with deterministic test-runner terminal input, expected stdout/stderr, and exit-code expectations. At first these tests are expected to be red because the proposed terminal features do not exist yet; terminal implementation then proceeds red-green-refactor until the projects gradually build and run successfully, and adoption is incomplete until every fixture below is green without weakening the project source.

After the Opalescent project sources and their harness expectations are written, but before terminal implementation begins, the project set MUST be checked with Oracle for scope, missing proposed syntax, accidental reliance on unimplemented non-terminal features, and definition-of-done clarity. Any Oracle finding that a project needs unsupported language/runtime facilities MUST be resolved by simplifying the project, not by expanding the terminal proposal. Oracle review is a planning gate only; it does not replace compiler, runtime, or integration-test evidence.

Every fixture MUST end with a deterministic summary line after closing or abandoning the terminal session so the existing integration style can assert behavior without snapshotting a whole live screen. Live rendering may be tested through exact output bytes only where the project already owns trusted reviewed terminal output. Each fixture MUST declare exhaustive `errors` clauses, use `guard` or `propagate` for every fallible call, and keep all terminal state in explicit loops and affine `using` scopes rather than closures or callbacks.

Required project fixtures:

1. **`terminal-key-log`** — Opens one session, consumes a bounded scripted sequence of Key, TimedOut, Cancelled, and EndOfInput outcomes, and prints a sanitized event log plus a final count summary. Done when the test proves Key payload refinement, modifier inspection, timeout handling, cancellation handling, EOF handling, clean close, and no direct string write to session output.
2. **`terminal-text-echo-safe`** — Echoes only committed `TextInput` through explicit trusted-output conversion while counting paste, unknown, reset, and non-text events separately. Done when the green project source contains no direct `string` session writes, companion compile-fail/security tests reject direct `string` session writes, trusted conversion is the only rendering boundary, and the final summary proves non-text input was not echoed.
3. **`terminal-size-probe`** — Opens a session, reads capabilities and terminal size, writes `columns x rows`, and closes. Done when constrained size types, capability snapshot inspection, trusted output conversion, flush, close outcome, and deterministic summary output are covered.
4. **`terminal-pause-counter`** — Reads a few events, calls `terminal_session_pause_sync`, iterates `TerminalPauseEvents` by index through the required final `InputReset.PauseBoundary`, and exits without resuming. Done when pause delivery, indexed pause-event access, final boundary validation, incremental-state clearing, and cleanup from `Paused` are verified.
5. **`terminal-legacy-io-rejection`** — Opens a session, intentionally calls future-fallible legacy input, handle creation, capability inspection, write, flush, and cursor/screen mutation operations, including at least `take_input`, `stdout_writer`, `stdout_terminal`, `terminal_supports_ansi`, one writer write/flush operation, and one terminal cursor or clear-screen operation. Done when each call records its structured coordinator-unavailable failure, and the tests prove rejection-before-consumption/mutation, stale raw-handle prevention, diagnostic-lane behavior, and the legacy state matrix.
6. **`terminal-timeout-menu`** — Displays a small numbered menu, polls with `TerminalWait.For`, chooses a default on timeout, and exits. Done when finite waits, `TimedOut` as an ordinary terminal event, bounded drain after readiness, trusted menu rendering, and deterministic default selection are verified.
7. **`terminal-cancel-demo`** — Creates a cancellation source/token, enters a session read loop, observes deterministic cancellation from the test backend, and exits through normal cleanup. Done when sticky cancellation, token use with `terminal_session_read_event_sync`, no new input consumption after cancellation wins, and summary reporting are verified.
8. **`terminal-diagnostics-inspector`** — Requests an unsupported or invalid terminal option, refines the resulting typed error, inspects all structured diagnostic fields, formats diagnostics safely, and exits without a session when appropriate. Done when typed error refinement, diagnostic inspectors, safe diagnostic formatting, absence of authority in formatted diagnostic text, and explicit propagation of unhandled families are covered.
9. **`terminal-chord-quit`** — Builds a minimal chord router from capabilities, registers Ctrl-Q and Escape, processes a bounded scripted stream, activates exactly one command, releases non-command input once, and exits. Done when router construction, binding IDs, modifier matching, activation, released input iteration, timer handling for pending prefixes, and no callback/application-payload storage in the router are verified.
10. **`terminal-paste-quarantine`** — Reads direct text, trusted paste, malformed paste fallback, and unknown bytes, then reports trusted chunks versus quarantined bytes. Done when trusted-paste evidence, malformed-frame quarantine, `UnknownBytes`, reset ordering, and non-command treatment of untrusted input are verified.
11. **`terminal-game-of-life-interactive`** — Starts from the existing `game-of-life-full` style but adds only simple key controls: pause/resume, speed up, slow down, deterministic scripted reseed, and quit. Done when sustained trusted frame rendering, frame-clock or proposed wait/timer interaction, Key handling, bounded event drains between frames, clean terminal restoration, and a deterministic final generation/state summary are verified. The fixture MUST NOT rely on randomness or real-time duration for expected output.
12. **`terminal-sokoban-mini`** — Implements a tiny fixed-board Sokoban puzzle with arrow-key movement, redraw, reset, and quit. Done when named-key handling, zero-based grid updates, repeated screen redraw/flush, illegal-move rejection, resize-safe rendering policy or explicit fixed-size assumption, and final move/box-position summary are verified.
13. **`terminal-file-picker`** — Lists a deterministic fixture directory from a fixed manifest or explicitly ordered fixture list, supports up/down movement and Enter selection, and prints the selected file after close. Done when filesystem integration, terminal key navigation, highlighted-row rendering through trusted output, cancellation/quit behavior, deterministic selection summary, and no editor-buffer dependency are verified. The fixture MUST NOT depend on unspecified host directory iteration order or an unimplemented sorting API.
14. **`terminal-stopwatch-pomodoro`** — Implements a simple stopwatch or Pomodoro-style loop with start/stop/reset/quit keys and deterministic short test ticks, not real Pomodoro-duration waits. Done when monotonic timer readiness, stale wake filtering, cancellation or quit handling, repeated flush, pause-free cleanup, and deterministic elapsed-tick summary are verified.

These projects are intentionally small applications, not proposal sketches. If a fixture cannot be written first in Opalescent using only currently implemented language features plus the proposed terminal features and proposed new syntax, the fixture MUST be reduced until it can; the implementation MUST NOT add unrelated language features merely to make these fixtures possible.

...
Every factory requires test-runner authority or an authority-derived scenario. Scenario creation requires `TerminalTestScenarioLimits`, whose `TerminalSessionResourceLimits` snapshot undergoes the same production cross-field validation as `terminal_session_options_validate` before any scenario, event-ID, composition-ID, evidence, backend-binding, activation state, stream identity, hidden correlated limit, or delivery-order state is created. Invalid resource relationships return `TerminalSessionOptionsError` without mutation. Successful creation issues one hidden test-runner-only input-stream identity, binds the exact validated `resource_limits.maximum_correlated_events` to that identity, and initializes its next delivery ordinal to one. Every `TerminalCapabilities` snapshot and every sealed `TerminalInputEvent` produced from that scenario carries the same identity, while every snapshot also carries that same hidden bound correlated-event limit. The production chord router authenticates the snapshot and derives the limit internally; tests cannot inspect, replace, or separately supply either hidden value. Events from another scenario exercise authentic wrong-stream rejection without a forgeable production identity. Neither hidden value has a test or production inspector or enters a production artifact. A successful immutable limits snapshot governs every synthetic text, preedit, paste, unknown-byte, size, correlation, retention, and diagnostic bound for that scenario. There is no undefined active-scenario limit, implicit fallback, or sentinel. A value rejected by that snapshot or by production construction is rejected here rather than normalized or clamped.
...
This document owns the exact compiler-checkable registrations for its test-only affine resources:

| Affine resource | Declared cleanup operation | Declared cleanup errors | Declared cleanup-authority transfer |
|---|---|---|---|
| `TerminalTestScenario` | compiler-only `terminal_test_scenario_drop` | explicit empty set | explicit empty set |
| `TerminalTestBackendActivation` | compiler-only `terminal_test_backend_activation_drop` | explicit empty set | explicit empty set |

Both cleanup operations are inaccessible ordinary source calls. `terminal_test_scenario_drop` infallibly releases that scenario's synthetic correlation, evidence, identifier, stream/order, and bound fake-backend state after nested activations have ended. `terminal_test_backend_activation_drop` has the exact LIFO behavior below. Neither operation advances production state, and no other operation consumes or transfers either owner through a borrow.
